...
namespace MShare_ASP.Services
{
    internal class OptimizedService : IOptimizedService
    {
        ...
        internal class DebtMatrix {
            ...
            public void UpdateDebt(long debtorId, long creditorId, long amount)
            {
        ...
        private async Task SaveDebtMatrix(long groupId, DebtMatrix debtMatrix)
        {
            using (var transaction = Context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (await Context.SaveChangesAsync() != oldOptimizedDebts.Count)
                    ...
                    if (await Context.SaveChangesAsync() != daoOptimizedDebts.Count)
                        throw new DatabaseException("todo");
        ...
        public async Task OptimizeForAllGroup() {
            ...
            foreach (var groupId in groupIds) {
                await OptimizeForGroup(groupId);
            }
        }
            ...
            foreach (var userId in userIds) {
                ...
                foreach (var userSpending in userSpendings) {
                    foreach (var daoDebtor in userSpending.Debtors) {
                        debtMatrix.UpdateDebt(daoDebtor.DebtorUserId, userId, daoDebtor.Debt ?? 0);
                    }
                }
            }
